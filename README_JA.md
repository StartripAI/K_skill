...
K.skill は、その断片をあとで開いて話せる persona pack にまとめます。voice note、チャットログ、スクリーンショット、キャラクター画像、世界観、公開文章、個人の原則を入れると、見返せて、会話を続けられて、必要な場所へ持ち出せる形になります。文字は出来事をほどき、声はその場に残っていた温度を戻します。
...
一枚の写真と音声クリップを、自分のマシンの中だけで短い喋るポートレート動画にできます。Voice Studio が生成した音声がポートレートを動かし、結果は何度でも見返せて pack に入れて持ち出せる、単体で完結した MP4 になります。内蔵レンダラーは、動画ツールが入っているコンピュータならグラフィックカードがなくても本物のクリップを作り、ハードウェアが対応していれば、より高精細な talking-head モデルをローカルで差し込めます。
...
Private chats stay outside the repository. 外部 provider を明示的に設定しない限り、素材は自分のマシンに残ります。

K.skill は動かすデバイスに合わせて調整します。いちばん軽い構成では文字と声にとどまり、喋るポートレートのような豊かなメディアは、スマートフォン級のブラウザからワークステーションまで、マシンが許す範囲で解放されます。判定はデバイス上で行われ、tier は手動で上書きできます。
